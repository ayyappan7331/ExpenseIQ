...
export const api = {
  ...
  // Transactions
  getTransactions: (opts: { profileId?: ProfileId; month?: string } = {}) =>
  ...
  // Subscriptions
  getSubscriptions: (opts: { profileId?: ProfileId } = {}) => subscriptionsApi.getAll(opts),
  ...
  // Debts
  getDebts: (opts: { profileId?: ProfileId } = {}) => debtsApi.getAll(opts),
  ...
  // Goals
  getGoals: (opts: { profileId?: ProfileId } = {}) => goalsApi.getAll(opts),
  upsertGoal: (data: NewGoal) => goalsApi.upsert(data),
  deleteGoal: (id: string) => goalsApi.delete(id),

  // Profiles
  getProfiles: () => profilesApi.getAll(),
  createProfile: (data: NewProfile) => profilesApi.create(data),
  deleteProfile: (id: ProfileId) => profilesApi.delete(id),

  // Credit cards
  getCreditCards: (opts: { profileId?: ProfileId } = {}) => creditCardsApi.getAll(opts),
  getArchivedCreditCards: (opts: { profileId?: ProfileId } = {}) => creditCardsApi.getArchived(opts),
  ...
  // Settings
  getSettings: (opts: { profileId?: ProfileId } = {}) => settingsApi.get(opts),
  updateSettings: (data: SettingsUpdate) => settingsApi.update(data),
  getDbStats: () => settingsApi.getDbStats(),

  // Budgets
  getBudgets: (opts: { profileId?: ProfileId; month?: string } = {}) => budgetsApi.getAll(opts),
  upsertBudget: (data: NewBudget) => budgetsApi.upsert(data),
  deleteBudget: (id: string) => budgetsApi.delete(id),

  // FinancialConfig (E4.5)
  getFinancialConfig: (opts: { profileId?: ProfileId } = {}) => financialConfigApi.get(opts),
  updateFinancialConfig: (data: Partial<FinancialConfig> & { profileId: ProfileId }) =>
