...
describe('budgetService', () => {
  ...
  it('upsert updates existing on same (profileId, month, category)', async () => {
    await service.upsert({ profileId: 'default', month: '2026-05', category: 'Food', amount: 5000 });
  ...
  it('upsert throws httpError(400) when month/category/amount missing', async () => {
    await expect(service.upsert({ profileId: 'default' })).rejects.toMatchObject({
      statusCode: 400,
      message: 'month, category, and amount are required',
    });
    await expect(
      service.upsert({ profileId: 'default', month: '2026-05' })
    ).rejects.toMatchObject({ statusCode: 400 });
    await expect(
      service.upsert({ profileId: 'default', month: '2026-05', category: 'Food' })
    ).rejects.toMatchObject({ statusCode: 400 });
  });
  ...
  it('findAll filters by month', async () => {
    await service.upsert({ profileId: 'default', month: '2026-05', category: 'Food', amount: 1 });
    await service.upsert({ profileId: 'default', month: '2026-06', category: 'Food', amount: 2 });
